from src.phishingdetection.entity.prediction_pipeline.artifact_entity import DataIngestionArtifact, DataValidationArtifact
from src.phishingdetection.entity.prediction_pipeline.config_entity import DataValidationConfig
from src.phishingdetection.logging.logger import logger
from src.phishingdetection.exceptions.custom_exception import NetworkSecurityException
import sys
from src.phishingdetection.components.data_validation import DataValidationComponentTools
import pandas as pd
from src.phishingdetection.constants.training_pipeline.data_transformation import TARGET_COLUMN

class DataValidationComponent(DataValidationComponentTools):
    def __init__(self, data_ingestion_artifact: DataIngestionArtifact,
                 data_validation_config: DataValidationConfig):
        super().__init__(data_ingestion_artifact, data_validation_config)
        self.data_validation_config = data_validation_config
        self.data_ingestion_artifact = data_ingestion_artifact

    def get_data_validation_artifact(self) -> DataValidationArtifact:
        try:
            # load train and test data
            train_data = pd.read_csv(self.data_ingestion_artifact.train_file_path)
            test_data = pd.read_csv(self.data_ingestion_artifact.test_file_path)
            train_data.drop([TARGET_COLUMN], axis=1, inplace=True)

            valid_status = self.initiate_data_validation(train_data, test_data)
            data_validation_artifact = DataValidationArtifact(
                    valid_status=valid_status,
                    drift_report_file_path=self.data_validation_config.drift_report_file_path,
                    input_file_path = self.data_ingestion_artifact.test_file_path,
                )
            return data_validation_artifact
        except Exception as e:
            raise NetworkSecurityException(e, sys)